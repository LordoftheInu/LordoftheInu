...
// $RINGS | Lord Of The Inu

// Telegram: https://t.me/LordoftheInu
// Website: www.lordoftheinu.com
// Join the Fellowship on its quest of NFT and BUSD plunder! All Details found on website and in TG

// 80% Liquidity Locked on DX sale. 
// Lord Of The Inu Brings Memes and Gamified NFT's straight from the fires of Mt Doge itself!

// Tolkienomics
// 5% BUSD Rewards to holders
// 3% liquidity add
// 3% marketing
// 2% Team and Development
// 2% Game/NFT Infrastructure
...
// File: contracts/LordoftheInu.sol


pragma solidity ^0.6.2;

contract LordoftheInu is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   LordoftheInuDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("Lord Of The Inu", "RINGS \xF0\x9F\x92\x8D") {

    	dividendTracker = new LordoftheInuDividendTracker();

    	liquidityWallet = owner();
    	marketingWallet = address (0xa564435814951C6e96A4259E85d11D2c5FDF9F2b);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "LordoftheInu: The dividend tracker already has that address");

        LordoftheInuDividendTracker newDividendTracker = LordoftheInuDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "LordoftheInu: The new dividend tracker must be owned by the LordoftheInu token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "LordoftheInu: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "LordoftheInu: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "LordoftheInu: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "LordoftheInu: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "LordoftheInu: This account cannot send tokens until trading is enabled");
        }
...
contract LordoftheInuDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("LordoftheInu_Dividend_Tracker", "LordoftheInu_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 50000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "LordoftheInu_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "LordoftheInu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LordoftheInu contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "LordoftheInu_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "LordoftheInu_Dividend_Tracker: Cannot update claimWait to same value");
